...
class Board:
    ...
    def play_game(self, player_x, player_o):
        """Wrapper method that manages a game between two players, the players
        can be both human or both computer or one human one computer.
        
        Player 'X' gets to go first. The user can choose to play with computer
        or with human player in the main function below. If there is/are human
        players, the player is prompted to enter moves via the terminal, which
        are then played on the board.User input is validated to ensure that only
        valid column numbers are accepted. When the game terminates, the final
        state of the board is printed along with a message indicating which
        player won (or if the game ended in a tie).
        
        Parameters:
            player_x - the player for 'X' side, which is either a computer
                       player of type Player, or a 'human'
            player_o - the player for 'O' side, which is either a computer
                       player of type Player, or a 'human'
        ...
        while (    (not self.win_for("X")) 
            ...
            if current_player == 'human':
                ...
                while not self.allows_move(move):
                    move = int(input(current_side + "'s move: "))
                self.add_move(move, current_side)
                print("Human playing for %s plays at %d" % (current_side, move))
                print()
            else:
                move = current_player.next_move(self)
                self.add_move(move, current_side)
                print("Computer playing for %s plays at %d" %
                      (current_side, move))
